...
cars = [(make, details)
	for make, details in car_data.items()
		if (search_criteria[0] <= details[1]
			and search_criteria[1] <= details[2]
			and search_criteria[2] >= details[4])
]

sort_cars = sorted(cars, key=lambda n: n[1][4])

for car in sort_cars[:5]:
	print(car)
